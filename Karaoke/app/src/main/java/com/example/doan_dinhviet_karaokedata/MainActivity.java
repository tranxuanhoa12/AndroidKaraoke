...
public class MainActivity extends AppCompatActivity {
    ...
    private void addControl() {
        ...
        tab= (TabHost) findViewById(R.id.mytab);
        tab.setup();
        ...
        lv1 = (ListView) findViewById(R.id.lv1);    //CAN DUOC SUA LAI
    ...
    private void addTim() {
        edttim.addTextChangedListener(new TextWatcher() {
            ...
            private void getdata() {
                String input = edttim.getText().toString(); myarray1.clear();
                if (!edttim.getText().toString().equals("")) {
                    Cursor c = database.rawQuery("SELECT * FROM ArirangSongList WHERE TENBH1 LIKE '"+"%"+input+"%"+"' OR MABH LIKE '"+"%"+input+"%"+"'", null);
                    c.moveToFirst();
                    while(c.isAfterLast()==false)
                    {
    ...
    public void CopyDataBaseFromAsset() { // TODO Auto-generated method stub
        try {
            InputStream myInput;
